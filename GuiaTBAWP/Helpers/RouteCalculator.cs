...
namespace GuiaTBAWP.Helpers
{
    ...
    public sealed class RouteCalculator
    {
        ...
        #region Ctor
        public RouteCalculator(CredentialsProvider credentialsProvider, string to, string from, Dispatcher uiDispatcher, Action<RouteResponse> routeFound)
        {
        ...
        private void client_GeocodeCompleted(object sender, GeocodeCompletedEventArgs e)
        {
            try
            {
                var state = e.UserState as RoutingState;
                ...
                if (e.Result.ResponseSummary.StatusCode != Bing.Geocode.ResponseStatusCode.Success || e.Result.Results.Count == 0)
                {
                    lock (StateSync)
                    {
                        _geoFailed = true;
                ...
                lock (StateSync)
                {
                    ...
                    var result = e.Result.Results.First();
        ...
        /// </summary>
        private void CalculateRoute(IEnumerable<GeocodeResult> locations)
        {
